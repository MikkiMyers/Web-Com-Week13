...
@section('content')
    <nav class="app-cmp-link-cat">
        <ul>
            <li>
                <a href="{{ route('shops.view-products', ['shop' => $shop->code, ]) }}">&lt;Back</a>
            </li>
        </ul>
    </nav>
    ...
    <main>
        <form action="{{ route('shops.add-product', ['shop' => $shop->code, ]) }}" method="post">
            @csrf
            <table class="app-cmp-data-list app-cl-common-link-container">
                <caption>Products for adding to {{ $shop->name }} </caption>
                <colgroup>
                    <col style="width: 0px">
                    <col>
                    <col style="width: 0px">
